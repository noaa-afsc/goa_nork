...
model_data::model_data(int argc,char * argv[]) : ad_comm(argc,argv)
{
 ...
 ad_comm::change_datafile_name("spp_catch.dat");
  nyrs_catch.allocate("nyrs_catch");
 cout<<"First year, and number of years catch is pre-specified. "<<styr<<" "<<nyrs_catch<<endl;
  nspp.allocate("nspp");
  ...
  Obs_Catch.allocate(1,nyrs_catch,0,nspp,"Obs_Catch");
 ...
 srprior_b = 10;
  ...
  pmaturetmp.allocate(1,nspp,1,69);
 ...
 cout<<"ABC: "<<SPR_abc<< endl<<SPR_ofl<<endl;
  // Open species-specific level details
   for (int i=1;i<=nspp;i++)
   {
     ...
     *(ad_comm::global_datafile) >> nages(i);                // 12
     ...
     for (int k=1;k<=nages(i);k++)
       *(ad_comm::global_datafile) >> pmaturetmp(i,k);       // 16
      cout << "Mature:  "<< pmaturetmp(i)(1,nages(i)) <<endl;
     for (int k=1;k<=nages(i);k++)
  ...
  pmature.allocate(1,nspp,1,nages);
  ...
  cout<<R<<endl;
  wt_mature.allocate(1,nspp,1,nages);
 wt_mature =  elem_prod(wt_F,pmature); 
 ...
 for (int i=1;i<=nspp;i++) Bcurrent(i) = pmature(i)*elem_prod(n0_F(i),wt_F(i));
 alpha = 0.05;
 alphaCI = 0.1;
 LCI =  1 + int(      alphaCI*.5  *nsims);
 UCI = -1 + int( (1 - alphaCI*.5) *nsims);
  ...
  F_begin_yr.allocate(1,nyrs_catch,1,nspp);
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  log_Rzero.allocate(1,nspp,0,13,1,"log_Rzero");
  steepness.allocate(1,nspp,0.2,1.0,2,"steepness");
  sigr.allocate(1,nspp,0.1,2.,3,"sigr");
  dummy.allocate(5,"dummy");
  ...
  // Simulate inv-gaussian RV's
    for (int i=1;i<=nsims;i++)
    {
      for (j=1;j<=npro+1;j++)
      {
        double psi   = value(square( rnorms(i,j) ));
...
void model_parameters::Run_Sim(void)
{
  ...
  ofstream& detail_out= *pad_detail_out;
  ofstream& prof_F= *pad_prof_F;
  detail_out<<"Alternative Spp Yr  ABC OFL Catch SSB F Tot_biom"<<endl;
    for (int ispp=1;ispp<=nspp;ispp++)
    {
...
void model_parameters::compute_obj_fun(void)
{
  ...
  for (int ispp=1;ispp<=nspp;ispp++)
  {
    ...
    if(Fmsy_F35>0) 
    {
      ...
      switch (current_phase())
      {
        case 1 : tmp1 +=  1.e1*square(log(Bmsy(ispp))-log(.35*SB100(ispp))); break;
        case 2 : tmp1 +=  1.e2*square(log(Bmsy(ispp))-log(.35*SB100(ispp))); break;
        case 3 : tmp1 +=  1.e2*square(log(Bmsy(ispp))-log(.35*SB100(ispp))); break;
        case 4 : tmp1 +=  1.e3*square(log(Bmsy(ispp))-log(.35*SB100(ispp))); break;
        default : tmp1+=  1.e3*square(log(Bmsy(ispp))-log(.35*SB100(ispp))); break;
      }
      obj_fun += tmp1;
    }
    if (Rec_Cond>0.)
...
void model_parameters::opt_sim(void)
{
  ...
  for (int isim=1;isim<=nsims;isim++)         // +++++++++++Sim
  {
    dmatrix Z_F(1,nspp,1,nages) ;
    dmatrix Z_M(1,nspp,1,nages) ;
    dmatrix S_F(1,nspp,1,nages) ;
    dmatrix S_M(1,nspp,1,nages) ;
    dmatrix Nnext_F(1,nspp,1,nages) ;
    dmatrix Nnext_M(1,nspp,1,nages) ;
  ...
  ofstream& prof_F= *pad_prof_F;
    ...
    { // call and get TAC's fo this species in this year...
      ...
      for (int ispp=1;ispp<=nspp;ispp++) 
      {
        ...
        OFL(ispp)       =    Get_Catch(6,ispp); // ABC_multiplier is from setup.dat
      }
      ...
      { // Build in condition to get TAC different than ABC...
        if (ipro==1)
        {
          for (int ispp=1;ispp<=nspp;ispp++) 
            Actual_Catch(ispp) = Obs_Catch(ipro,ispp); 
            ...
            if (alt == 6 || alt == 7) 
            {
              ...
              if (alt == 7)
              {
                if (ipro<=3)
                {
                  ...
                  // cout<<alt<<" "<<ipro<<" "<<Actual_Catch<<" "<<Get_F(1,1)<<endl;
                }
                else 
                  for (int ispp=1;ispp<=nspp;ispp++) 
        ...
        ABCs_by_yr(ipro) += ABC; // Cumulate ABCs here for printout later...
        TACs_by_yr(ipro) += TAC; // Cumulate TACs here for printout later...
      }
      Project_Pops(isim,ipro);
      //cout << "Done projection"<<endl; //cout <<"Csim: "<<isim<<" "<<i<<" "<<Csim(1,isim,i) <<" "<< endl; //cout << "Yr_"<<i+styr-1<<"_Expl "<< Expl_Biom<<endl;
      for (int ispp=1;ispp<=nspp;ispp++) 
      {
        detail_out<<alt<<" "<<spname(ispp)<<" "<<styr+ipro-1<<" "<<ABC(ispp)<<" "<<OFL(ispp)<<" "<<Actual_Catch(ispp)<<" "<<SBsim(ispp,isim,ipro)<<" "<<Fsim(ispp,isim,ipro)  <<" "<<Bsim(ispp,isim,ipro)  << endl;
...
void model_parameters::Mainloop(const int& ispp)
{
  ofstream& means_out= *pad_means_out;
  ofstream& percent_out= *pad_percent_out;
  ofstream& percent_db= *pad_percent_db;
  ...
  //   called from main_loop from opt_sim                                     
...
double model_parameters::Get_Catch(const int& thisalt, const int& ispp)
{
  ...
  // cout<<spname(ispp)<<" Ftotabc "<<Ftotabc(ispp)(1,6)<<endl;
  // cout<<spname(ispp)<<" SSB "<<SBtmp<<" "<<endl;
  // Set a good starting guess for spawning biomass 
  SBtmp = (N_F(ispp) * elem_prod( wt_mature(ispp), mfexp( -yrfrac(ispp)*(M_F(ispp) + Ftotabc(ispp) )))); 
  // Calls Get_F_t to compute the current year's F (as a function of stock size, species etc)
  Ftmp = 0.0; 
  Ftmp = Get_F(thisalt, ispp);
  // cout <<"F "<<thisalt<<" "<<Ftmp<<endl;
  ...
  SBtmp = N_F(ispp) * elem_prod( wt_mature(ispp), mfexp( -yrfrac(ispp)*(M_F(ispp) + Ftottmp_F))); 
  ...
  // Test for FMP species else do basic F
  {
    switch (thisalt) 
    {
      ...
      case 2 : // Auth ABC
        ...
        break;
      case 3 : // Half max F
        ftmp = 0.5 * Get_F_t( F_age_tmp, N_F(ispp), ispp); 
        break;
      case 4 : 
        ftmp = avg_5yrF(ispp); 
        break;
      ...
      case 42 : 
        ...
        break;
      case 99 : 
        // Simply a constant rate--equals the OFL
        ftmp = (Fofl(ispp));
        break;
      default  :
...
double model_parameters::Get_F_SSL_prey(const dvector& F_age, const dvector& N_females, const int ispp )
{
  ...
  {
    if (SBtmp < 0.2*SB100(ispp))  // NOTE 20% of SB100%
      Ftmp = 0.;
    if (SBtmp >= 0.2 *SB100(ispp) & SBtmp < SBF40(ispp) )  // NOTE Same as Am 56 here (until 20% bzero reached)
      Ftmp = (Fabc(ispp)*(1/(1-alpha ))*(SBtmp/SBF40(ispp) - alpha ));
    if (SBtmp > SBF40(ispp) )
      Ftmp = (Fabc(ispp));
    SBtmp = N_females * elem_prod( wt_mature(ispp), mfexp( -yrfrac(ispp)*(M_F(ispp) + Ftmp*F_age ))); 
  }
  return(Ftmp);
}
...
double model_parameters::Get_F_Am56(const dvector& F_age, const dvector& N_females, const int ispp )
{
  ...
  double Ftmp; // cout<< spname(ispp)<< endl;
  {
    for (ii=1;ii<=3;ii++) // Iterate to get month of spawning correct
    {
      if (SBtmp < alpha*SBF40(ispp))
        Ftmp = 0.;
      if (SBtmp >= alpha*SBF40(ispp) & SBtmp < SBF40(ispp) )
        Ftmp = (Fabc(ispp)*(1/(1-alpha))*(SBtmp/SBF40(ispp) - alpha));
      if (SBtmp > SBF40(ispp) )
        Ftmp = (Fabc(ispp));
      SBtmp = N_females * elem_prod( wt_mature(ispp), mfexp( -yrfrac(ispp)*(M_F(ispp) + Ftmp*F_age ))); 
  ...
  double Ftmp; // cout<< spname(ispp)<< endl;
  {
    for (ii=1;ii<=3;ii++) // Iterate to get month of spawning correct
    {
      if (SBtmp < alpha*SBF40(ispp))
        Ftmp = 0.;
      if (SBtmp >= alpha*SBF40(ispp) & SBtmp < SBF40(ispp) )
        Ftmp = (Fofl(ispp)*(1/(1-alpha))*(SBtmp/SBF40(ispp) - alpha));
      if (SBtmp > SBF40(ispp) )
        Ftmp = (Fofl(ispp));
      SBtmp = N_females * elem_prod( wt_mature(ispp), mfexp( -yrfrac(ispp)*(M_F(ispp) + Ftmp*F_age ))); 
    }
  ...
  for (int ispp=1;ispp<=nspp;ispp++) // ++++++++++++++Species
  {
    // if ( ! isit_const(ispp) )
    {
      ...
      if (Ftmp > 0.)
      {
        ...
        SPRsim(ispp,isim,i) =  Implied_SPR(Ftottmp_F,ispp);
...
void model_parameters::get_SB100(void)
{
  ...
  for (int ispp=1;ispp<=nspp;ispp++)
  {
    ...
    // Kill them off till spawning 
      SB100(ispp)    += NsprF0(ispp,j)*pmature(ispp,j)*wt_F(ispp,j) * mfexp(-yrfrac(ispp) *M_F(ispp,j));
    }
...
void model_parameters::compute_spr_rates(void)
{
  ...
  for (int ispp=1;ispp<=nspp;ispp++)
  {
    ...
    {
    // Kill them off till spawning 
      SBFabc(ispp) += NsprFabc(ispp,j)*pmature(ispp,j)*wt_F(ispp,j)* mfexp(-yrfrac(ispp) *( M_F(ispp,j) + Ftotabc(ispp,j) ));
      SBF40(ispp)  +=  NsprF40(ispp,j)*pmature(ispp,j)*wt_F(ispp,j)* mfexp(-yrfrac(ispp) *( M_F(ispp,j) + Ftot40(ispp,j) ));
      SBFofl(ispp) += NsprFofl(ispp,j)*pmature(ispp,j)*wt_F(ispp,j)* mfexp(-yrfrac(ispp) *( M_F(ispp,j) + Ftotofl(ispp,j) ));
    }
    SBFabc(ispp) = .5 * AMeanRec(ispp) * SBFabc(ispp);
    SBF40(ispp)  = .5 * AMeanRec(ispp) *  SBF40(ispp);
    SBFofl(ispp) = .5 * AMeanRec(ispp) * SBFofl(ispp);
    BFabc(ispp)  = .5 * AMeanRec(ispp) * (NsprFabc(ispp)*wt_F(ispp) + NsprMabc(ispp)*wt_M(ispp) ) ;
    BF40(ispp)   = .5 * AMeanRec(ispp) *  (NsprF40(ispp)*wt_F(ispp) +  NsprM40(ispp)*wt_M(ispp) ) ;
    BFofl(ispp)  = .5 * AMeanRec(ispp) * (NsprFofl(ispp)*wt_F(ispp) + NsprMofl(ispp)*wt_M(ispp) ) ;
  }
  // cout << setprecision(2) <<Fabc<<endl
}
...
double model_parameters::Implied_SPR( const dvector& F_age, const int& ispp)
{
  ...
  // Function that returns SPR percentage given a realized value of F...
    ...
    {
    // Kill them off till spawning 
      sb0_tmp      += ntmp0(j)*pmature(ispp,j)*wt_F(ispp,j) * mfexp(-yrfrac(ispp) *M_F(ispp,j));
      sb1_tmp      += ntmp1(j)*pmature(ispp,j)*wt_F(ispp,j)* mfexp(-yrfrac(ispp) *( M_F(ispp,j) + F_age(j) ));
    }
...
double model_parameters::SolveF2(const dvector& N_F, const dvector& N_M, const double&  TACin, const int& ispp)
{
  ...
  while (dd > 1e-6)
  {
    ...
    if (iter>50) {cerr<<"Bombed on catch solver--check scales for "<<spname(ispp)<<" Catch was "<<TACin<<endl<<" Biomass: "<<btmp<<endl<<" Catch, F trial was: "<<cc<<" "<<ftmp<<" "<<dd<<endl<<sel_F(ispp);exit(1);}
  }
  return(ftmp);
}
...
void model_parameters::Get_SPR_Catches(const int& ispp)
{
  ...
  for (m=1;m<=ngear(ispp);m++)
  {
    Cabc(ispp) += ( wt_gear_F(ispp,m) * .5* AMeanRec(ispp)*elem_prod(elem_div( Fabc(ispp)*Frat(ispp,m)*sel_F(ispp,m) ,
                  M_F(ispp) + Ftotabc(ispp)),elem_prod(1.- mfexp(-(M_F(ispp)+Ftotabc(ispp))),NsprFabc(ispp))));
    Cabc(ispp) += ( wt_gear_M(ispp,m) * .5* AMeanRec(ispp)*elem_prod(elem_div( Fabc(ispp)*Frat(ispp,m)*sel_M(ispp,m) , 
                  M_M(ispp) + Ftotabc(ispp)),elem_prod(1.- mfexp(-(M_M(ispp)+Ftotabc(ispp))),NsprMabc(ispp))));
    Cofl(ispp) += ( wt_gear_F(ispp,m) * .5* AMeanRec(ispp)*elem_prod(elem_div( Fofl(ispp)*Frat(ispp,m)*sel_F(ispp,m) , 
                  M_F(ispp) + Ftotofl(ispp)),elem_prod(1.- mfexp(-(M_F(ispp)+Ftotofl(ispp))),NsprFofl(ispp))));
    Cofl(ispp) += ( wt_gear_M(ispp,m) * .5* AMeanRec(ispp)*elem_prod(elem_div( Fofl(ispp)*Frat(ispp,m)*sel_M(ispp,m) , 
 ...
 adstring ad_tmp=initial_params::get_reportfile_name();
  ...
  for (int ispp=1;ispp<=nspp;ispp++) 
  {
    ...
    for (int ii=1;ii<=4*nages(ispp);ii++)
    {
      if (ii>nages(ispp))
      {
        Tg += double(ii) * wt_mature(ispp,nages(ispp)) * ntmp;
        tmp +=  wt_mature(ispp,nages(ispp)) * ntmp;
        ntmp *= exp(-M_F(ispp,nages(ispp)));
      }
      else
      {
        Tg += double(ii) * wt_mature(ispp,ii) * ntmp;
        tmp +=  wt_mature(ispp,ii) * ntmp;
...
void model_parameters::Do_Sims(void)
{
  ...
  // Main optimization simulation here...
    ...
    write_by_time();
    //cout << Bsim(1)<<endl<<endl<<Csim(1)<<endl<<endl<<Fsim(1)<<endl;exit(1);
...
void model_parameters::write_avg_age(const adstring& Title)
{
  ...
  means_out << "AvgAgeYr"<< styr + npro -1 << " ";
    for (int ispp=1;ispp<=nspp;ispp++) means_out << Avg_Age_End(ispp)/nsims<< " "; means_out << endl;
}
...
void model_parameters::Get_Bzero(const int& ispp)
{
  ...
  Bzero(ispp) = 0.5* wt_mature(ispp)  * Ntmp ; // p_mature is of Females (half of adults)
  switch (SrType)
  {
    case 1:
...
void model_parameters::Recruitment_Likelihood(const int& ispp)
{
  ...
  // Tune recruits to spawners via functional form of Srec (to estimate srec params) RAM's exp. value form of -ln like
    rec_like(ispp) = (norm2( log(R(ispp)+1.e-8) - 
                             ...
                             (2.*sigmaRsq(ispp))) + nrec(ispp) * log(sigmaRsq(ispp));
}
...
dvariable model_parameters::yield(const dvariable& Ftmp, dvariable& Stmp,dvariable& Rtmp,const int& ispp)
{
  ...
  phitmp = elem_prod( Ntmp_F , pow(survtmp_F,yrfrac(ispp)) ) * wt_mature(ispp)  ; //Req    = Requil(phi);
...
double model_parameters::get_spr_rates(double spr_percent,int& ispp)
{
  ...
  F1 = 0.0; 
  F1 = .8*M_F(ispp,2); // Start at some fraction of natural mortality
  double F2;
  double F3;
  double yld1;
  double yld2;
...
double model_parameters::spr_ratio(double& trial_F,dmatrix& sel_tmp_F, int& ispp)
{
  ...
  Ntmp(1)=1.;
  j=1;
  SBtmp  += Ntmp(j)*wt_mature(ispp,j)*pow(srvtmp_F(j),yrfrac(ispp));
  for (j=2;j<nages(ispp);j++)
  {
  ...
  SBtmp  += Ntmp(nages(ispp))*wt_mature(ispp,nages(ispp))*pow(srvtmp_F(nages(ispp)),yrfrac(ispp));
  SBtmp *= .5 * AMeanRec(ispp) ;
